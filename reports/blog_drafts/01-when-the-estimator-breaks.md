...
*Draft. Target: a short lab note, 800–1200 words.*
...
throughput by 30–66 %. The error grew monotonically with batch size:
...
Then I ran the same model on MPS — Apple's PyTorch GPU backend —
with a slightly wider sweep, using the catalogued M3-Max peak
bandwidth of 400 GB/s.

The picture flipped:

- `seq=64`: formula too high by 70–94 %. (Launch overhead dominates;
  the formula has no fixed-overhead term.)
- `seq=128`: formula too high by ~40–50 %.
- `seq=256`: formula too **low** by 16–43 %.
- `seq=512, batch ∈ {1, 2, 4}`: formula too low by **95–154 %**.
...
(a) the "peak memory bandwidth" number I plugged in (400 GB/s)
...
making — "should I rent an A100 or an H100?", "is this batch size
going to work?", "how long will this training run actually take?".
You only trust a tool like that if you have calibrated it against
real measurements, and you have written down where you stopped
trusting it.
